...
### 1.1 Zero-Downtime Migration (Standard NFTs → Compressed NFTs)
...
├── Both v1.0 (standard) and v2.0 (cNFT) programs live
├── Users can choose which version to use
└── No forced migration

Day 30-60: Migration Incentives
├── Offer ZEN token bonuses for migrating
├── Enable migration tool in frontend
└── Communicate benefits (lower costs, new features)

Day 60-90: Sunset Warning
├── Announce v1.0 deprecation timeline
├── Final migration push with 2x bonuses
└── Set read-only mode for v1.0

Day 90+: Full Migration
├── v1.0 program set to read-only
├── All new mints use v2.0
└── Legacy beasts still queryable
```
...
export class BeastMigrator {
  ...
  private async recordMigration(v1: PublicKey, v2: PublicKey): Promise<void> {
    ...
    console.log(`Migration: ${v1.toString()} → ${v2.toString()}`);
  }
  ...
  const handleMigrate = async () => {
    ...
    for (let i = 0; i < v1Beasts.length; i++) {
      ...
      try {
        ...
        console.log(`Migrated: ${v1Beasts[i]} → ${v2Mint}`);
...
async function importBeasts() {
  ...
  for (const beast of data) {
    try {
      ...
      console.log(`✓ Imported ${beast.mint}`);
    } catch (err) {
      console.error(`✗ Failed to import ${beast.mint}:`, err);
    }
  }
}
